...
void VulkanEngine::init_pipelines() {
  ...
  VK_CHECK(vkCreatePipelineLayout(_device, &test_pipeline_layout_info, nullptr,
                                  &_opengl_layout));
...
void VulkanEngine::load_images() {
  ...
  VkImageViewCreateInfo imageinfo = vkinit::imageview_create_info(
      VK_FORMAT_R8G8B8A8_SRGB, lostEmpire.image._image,
...
void VulkanEngine::update_material_pipeline(VkPipeline pipeline,
                                            VkPipelineLayout layout,
                                            const std::string &name) {
...
void VulkanEngine::init_scene() {
  ...
  map.transformMatrix =
      glm::translate(glm::vec3{5, -10, 0}); // glm::mat4{ 1.0f };
...
void VulkanEngine::init_descriptors() {
  ...
  for (int i = 0; i < FRAME_OVERLAP; i++) {
    ...
    _openglFrames[i].objectBuffer = create_buffer(
    ...
    vkutil::DescriptorBuilder::begin(_frames[i].layoutCacheAllocator,
        ...
        .bind_buffer(1, &sceneInfo, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER_DYNAMIC,
                     VK_SHADER_STAGE_FRAGMENT_BIT)
